...
let build_team () =
  let rec add_pokemon curr_team =
    print_string "Choose a species: ";
    let spec = read_line () in
    print_string "Choose a nature: ";
    let nat = read_line () in
    try
      let poke = create spec 1 nat in
      let new_team = poke :: curr_team in
      print_endline
        ("Your level 1 " ^ nat ^ " " ^ spec ^ " was added to the team!");
      if List.length new_team >= 4 then (
        print_endline "You've created a full team!";
        print_endline (String.concat ", " (List.map species new_team));
        new_team)
      else add_pokemon new_team
    with BadPokemon ->
      print_endline "That is not a valid Pokemon. Try again.";
...
let () =
  print_endline "Welcome to Pokemon!";
